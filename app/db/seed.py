...
def seed_admin_user(db: Session) -> None:
    """Opt-in: only runs when ADMIN_EMAIL/ADMIN_PASSWORD are set in the env.

    Safe to leave configured across restarts: if the account already exists
    it is just promoted (never re-created, never has its password reset), so
    an admin who changed their password later does not get overwritten.
    """
    ...
    if not settings.ADMIN_PASSWORD:
        logger.warning(
            "ADMIN_EMAIL is set but ADMIN_PASSWORD is empty; skipping admin seed"
        )
        return
    ...
    if user:
        if not user.is_admin:
            user.is_admin = True
            db.commit()
            logger.info("Promoted existing account to admin: %s", email)
        return
